...
from prompt.algo_code import tsp_dynamic_programming,tsp_greedy_nearest_neighbor,graph_coloring_backtracking,vertex_cover_brute_force,dijkstra_shortest_path
...
SIZE = {
    "TSP_data_50":["graph_{index}.json".format(index = i) for i in range(4,25)],
    "Coloring_data_50":["graph_{index}.json".format(index = i) for i in range(8,26)],
    "vertexcover_data_50":["graph_{index}.json".format(index = i) for i in range(8,26)],
    "shortest_path_data":["graph_{index}.json".format(index = i) for i in range(3,26)],
}
IMPO = """
***No additional information needs to be generated; only a complete dictionary is required.***
"""

def generate_adjacency_list(nodes: list, edges: list) -> dict:
    r"""Generates an adjacency list and node mapping from the given graph input.

    Args:
        nodes (list): A list of node names (strings).
        edges (list): A list of edges, where each edge is a tuple
...
def save_result(problem_type,save_dir,result_list,file_name,model_name,epcoh_file_name):
    save_dir_final = SAVE_RESULT_DIR + epcoh_file_name + "/" + model_name + "/" + problem_type + "/" + save_dir + "/" + file_name
    path = Path(SAVE_RESULT_DIR + epcoh_file_name + "/" +  model_name + "/" + problem_type + "/" + save_dir + "/")
    if not path.exists():
...
def reasoning(problem_type,Q,agent_list,graph_index,file_name,adj,name_list,result_dic,model_name,real_result,epcoh_file_name):
    ...
    for singel_agent in agent_list:
        ...
        if try_num == try_limited:
            ...
            save_result(problem_type,agent_name,result_dic[agent_name],file_name,model_name,epcoh_file_name)
            continue
        end_time = time.time()
    ...
    if model_name == "qwen2.5-14b":
        base_model = ModelFactory.create(
         ...
         base_model = ModelFactory.create(
                    ...
                    api_key =  CLOSEAI_KEY)    
    elif model_name == "gpt4o-mini-closeai":
         base_model = ModelFactory.create(
                    model_platform=ModelPlatformType.OPENAI,
                    model_type=ModelType.GPT_4O_MINI,
                    url = 'https://api.openai-proxy.org/v1',
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=DATA_LIST,
        default='None',
        help="Dataset. Available options: {}".format(', '.join(available_models))
    )
    parser.add_argument(
        '--test_num',
        type=int,
    ...
    for epoch in range(epoch_num):
        epoch_file_name = "epoch_" + str(epoch)
        for data_name in data_list:
            ...
            graph_algo = [FunctionTool(func) for func in [tsp_dynamic_programming,tsp_greedy_nearest_neighbor,graph_coloring_backtracking,vertex_cover_brute_force,dijkstra_shortest_path]]
            for file in json_data_list:
                save_dir_final = SAVE_RESULT_DIR + "/" + epoch_file_name + "/" + model_name + "/" + data_name + "/Algorithm_Solving_Agent/" + file["file_name"]
                path = Path(save_dir_final)
                if path.exists():
                    print(file["file_name"]+"存在！跳过！")
                    continue
                if file["file_name"]  not in SIZE[data_name]:
                    continue
                # if file["file_name"] != "graph_25.json":
                #     continue
                ...
                for graph in graph_data:
                    ...
                    reasoning(data_name,question_msg,agent_list,graph["graph_index"],file["file_name"],graph["adj"],graph["name_list"],result_dic,model_name,graph["result"],epoch_file_name)
