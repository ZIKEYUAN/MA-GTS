...
from prompt.algo_code import tsp_dynamic_programming,tsp_greedy_nearest_neighbor,graph_coloring_backtracking,vertex_cover_brute_force,cycle_DFS
...
def save_result(problem_type,save_dir,result_list,file_name,model_name):
    save_dir_final = SAVE_RESULT_DIR + model_name + "/" + problem_type + "/" + save_dir + "/" + "1070_" + file_name
...
def reasoning(problem_type,Q,agent_list,graph_index,file_name,adj,name_list,result_dic,model_name,real_result,graph_difficulty):
...
def get_algo_base_memory():
    ...
    return memory
def create_base_model(model_name="gpt4o-mini"):
...
if __name__ == "__main__":
    ...
    for data_name in DATA_LIST:
        ...
        graph_algo = [FunctionTool(func) for func in [tsp_dynamic_programming,
        ...
        for file in json_data_list:
            ...
            for graph_index in graph_data:
                print(graph_index)
                if int(graph_index) < 1070:
                    continue
                graph = graph_data[graph_index]
                Q_context = graph["question"]
                agent_list = []
                ...
                reasoning(data_name,question_msg,agent_list,int(graph_index),file["file_name"],None,None,result_dic,model_name,graph["answer"],graph["difficulty"])
        #         break
        #     break
        # break